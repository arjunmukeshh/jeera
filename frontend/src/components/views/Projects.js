...
const Projects = () => {
    ...
    return (
        <div>
            <Header />
            <h1>Your Projects</h1>
            {isAdmin && <h2 className='inline modal-card'><Link to="/users">Users |</Link></h2>}
            {isAdmin && <h2 className='inline modal-card'><Link to="/teams"> Edit Teams | </Link></h2>}
            {isAdmin && <h2 className='inline modal-card'><Link to="/register"> Register User(s)</Link></h2>}<br /><br />
